...
# ========================== 用户配置 ==========================
SPIN_CHANNEL = 'up'               # 选择 'up' 或 'down'
FERMI_ENERGY = -2.2883            # 费米能级 (eV)，用于将能量平移至 E_F=0

# 单独偏移量（用于对齐 DFT 和 Wannier 能带）
# 如果 Wannier 能带整体偏高 0.3 eV，设为 -0.3
# 如果 Wannier 能带整体偏低 0.2 eV，设为 0.2
DFT_OFFSET = 0.0                  # DFT 能带偏移量 (eV)
WANNIER_OFFSET = 0.0              # Wannier 能带偏移量 (eV)

# 手动指定 Y 轴范围，例如 YLIM = (-2, 2) ；若设为 None 则自动确定
YLIM = (-3.5, 3)                    # 自定义能量范围，与 Fatband 图保持一致
...
def read_BAND(filename, spin='up'):
    ...
    while i < len(lines):
        ...
        if len(k_band) == nkpts:
            ...
            e_list.append(e_arr[sort_idx] - FERMI_ENERGY + DFT_OFFSET)
...
def read_wannier_band(filename):
    ...
    if data.ndim == 2 and data.shape[1] > 2:
        k = data[:, 0]
        energies = data[:, 1:] - FERMI_ENERGY + WANNIER_OFFSET
        return k, energies
        ...
        if len(split_idx) == 0:
            return k_all, (e_all - FERMI_ENERGY + WANNIER_OFFSET).reshape(-1, 1)
        ...
        energies = np.column_stack([e[:min_len] - FERMI_ENERGY + WANNIER_OFFSET for e in e_segments])
...
def main():
    ...
    if DFT_OFFSET != 0:
        print(f"DFT 偏移量: {DFT_OFFSET:.4f} eV")
    if WANNIER_OFFSET != 0:
        print(f"Wannier 偏移量: {WANNIER_OFFSET:.4f} eV")
